            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Edit Data for Soil Erosion Records</h4>     
                    </div>
                    <div class="card-body">
                    ...
                    <?php
                        if(isset($_GET['id']))
                        {
                            $erosion_id = mysqli_real_escape_string($con, $_GET['id']);
                            $query = "SELECT * FROM disaster_records WHERE id = '$erosion_id'";
                            
                            $query_run = mysqli_query($con,$query);

                            if(mysqli_num_rows($query_run) > 0)
                            {
                                $erosion = mysqli_fetch_array($query_run);
                        ?>

                        <form action="../../code.php" method="POST">

                            <input type="hidden" name="erosion_id" value="<?= $erosion['id']; ?>">        

                            <div class="mb-3">
                            <label> Barangay </label>
                                <select class="form-control form-select mb-3" aria-label=".form-select-lg example" name="barangay" required>
                                    <option value="<?=$erosion['barangay_name']; ?>"><?=$erosion['barangay_name']; ?></option>
                            ...
                            <div class="mb-3">
                                <label>Zone</label>
                                    <select class="form-select form-select mb-3" aria-label=".form-select-lg example" name="zone" required>
                                            <option value="<?=$erosion['zone']; ?>"><?=$erosion['zone']; ?></option>
                            ...
                            <div class="mb-3">
                                <label>Type of Disaster</label>
                                    <select class="form-control form-select mb-3" aria-label=".form-select-lg example" name="disaster" required>
                                        <option value="<?=$erosion['disaster']; ?>"><?=$erosion['disaster']; ?></option>               
                            ...
                            <div class="mb-3">
                                <label>No. of Soil Erosion Map</label>
                                    <select class="form-select form-select mb-3" aria-label=".form-select-lg example" name="number_map" required>
                                            <option value="<?=$erosion['number_map']; ?>"><?=$erosion['number_map']; ?></option>          
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="population_at_risk"  value="<?=$erosion['population_at_risk']; ?>" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label>Recommendation</label>
                                <input type="text" name="recommendation"  value="<?=$erosion['recommendation']; ?>" class="form-control">
                            </div>
                            <div class="mb-3">
                                <label>Remarks</label>
                                <input type="text" name="remarks" value="<?=$erosion['remarks']; ?>" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <button type="submit" name="update-erosion" class="btn btn-primary">Update</button>
                            </div>
