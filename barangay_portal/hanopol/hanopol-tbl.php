            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Barangay Hanopol Data
                            <a href="add-data.php" class="btn btn-primary float-end">Add Data</a>
                        </h4>
                    </div>
                    <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php 
                                    $query = "SELECT * FROM barangay_data WHERE barangay_name='Hanopol'";
                                    $query_run = mysqli_query($con, $query);

                                    if(mysqli_num_rows($query_run) > 0)
                                    {
                                        foreach($query_run as $hanopol)
                                        {
                                            ?>
                                            <tr>
                                                <td style="display:none;"><?= $hanopol['id']; ?></td>
                                                <td><?= $hanopol['zone']; ?></td>
                                                <td><?= $hanopol['male']; ?></td>
                                                <td><?= $hanopol['female']; ?></td>
                                                <td><?= $hanopol['youth']; ?></td>
                                                <td><?= $hanopol['senior_citizen']; ?></td>
                                                <td><?= $hanopol['pregnant']; ?></td>
                                                <td><?= $hanopol['pwd']; ?></td>
                                                <td><?= $hanopol['children']; ?></td>
                                                <td><?= $hanopol['no_evacuations']; ?></td>
                                                <td id="print-btn" class="col-md-1"><?= $hanopol['created_date']; ?></td>
                                                <td id="print-btn" class="col-md-1"><?= $hanopol['modified_date']; ?></td>
                                                <td id="print-btn"  class="col-md-1.1">
                                                    <a href="edit-data.php?id=<?= $hanopol['id']; ?>" class="btn btn-success btn">Edit</a>
