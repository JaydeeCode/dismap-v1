...
<link rel="stylesheet" type="text/css" href="../style-print.css" media="print" >
    ...
    <title>Barangay Cabacongan</title>
</head>
<body>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php 
                                    $query = "SELECT * FROM barangay_data WHERE barangay_name='Cabacongan'";
                                    $query_run = mysqli_query($con, $query);

                                    if(mysqli_num_rows($query_run) > 0)
                                    {
                                        foreach($query_run as $cabacongan)
                                        {
                                            ?>
                                            <tr>
                                                <td style="display:none;"><?= $cabacongan['id']; ?></td>
                                                <td><?= $cabacongan['zone']; ?></td>
                                                <td><?= $cabacongan['male']; ?></td>
                                                <td><?= $cabacongan['female']; ?></td>
                                                <td><?= $cabacongan['youth']; ?></td>
                                                <td><?= $cabacongan['senior_citizen']; ?></td>
                                                <td><?= $cabacongan['pregnant']; ?></td>
                                                <td><?= $cabacongan['pwd']; ?></td>
                                                <td><?= $cabacongan['children']; ?></td>
                                                <td><?= $cabacongan['no_evacuations']; ?></td>
                                                <td id="print-btn" class="col-md-1"><?= $cabacongan['created_date']; ?></td>
                                                <td id="print-btn" class="col-md-1"><?= $cabacongan['modified_date']; ?></td>
                                                <td id="print-btn"  class="col-md-1.1">
                                                    <a href="edit-data.php?id=<?= $cabacongan['id']; ?>" class="btn btn-success btn">Edit</a>
