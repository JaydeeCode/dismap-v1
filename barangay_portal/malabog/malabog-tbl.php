                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php 
                                    $query = "SELECT * FROM barangay_data WHERE barangay_name='Malabog'";
                                    $query_run = mysqli_query($con, $query);

                                    if(mysqli_num_rows($query_run) > 0)
                                    {
                                        foreach($query_run as $malabog)
                                        {
                                            ?>
                                            <tr>
                                                <td style="display:none;"><?= $malabog['id']; ?></td>
                                                <td><?= $malabog['zone']; ?></td>
                                                <td><?= $malabog['male']; ?></td>
                                                <td><?= $malabog['female']; ?></td>
                                                <td><?= $malabog['youth']; ?></td>
                                                <td><?= $malabog['senior_citizen']; ?></td>
                                                <td><?= $malabog['pregnant']; ?></td>
                                                <td><?= $malabog['pwd']; ?></td>
                                                <td><?= $malabog['children']; ?></td>
                                                <td><?= $malabog['no_evacuations']; ?></td>
                                                <td id="print-btn" class="col-md-1"><?= $malabog['created_date']; ?></td>
                                                <td id="print-btn" class="col-md-1"><?= $malabog['modified_date']; ?></td>
                                                <td id="print-btn"  class="col-md-1.1">
                                                    <a href="edit-data.php?id=<?= $malabog['id']; ?>" class="btn btn-success btn">Edit</a>
