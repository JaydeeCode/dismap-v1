                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php 
                                    $query = "SELECT * FROM barangay_data WHERE barangay_name='Tabgon'";
                                    $query_run = mysqli_query($con, $query);

                                    if(mysqli_num_rows($query_run) > 0)
                                    {
                                        foreach($query_run as $tabgon)
                                        {
                                            ?>
                                            <tr>
                                                <td style="display:none;"><?= $tabgon['id']; ?></td>
                                                <td><?= $tabgon['zone']; ?></td>
                                                <td><?= $tabgon['male']; ?></td>
                                                <td><?= $tabgon['female']; ?></td>
                                                <td><?= $tabgon['youth']; ?></td>
                                                <td><?= $tabgon['senior_citizen']; ?></td>
                                                <td><?= $tabgon['pregnant']; ?></td>
                                                <td><?= $tabgon['pwd']; ?></td>
                                                <td><?= $tabgon['children']; ?></td>
                                                <td><?= $tabgon['no_evacuations']; ?></td>
                                                <td id="print-btn" class="col-md-1"><?= $tabgon['created_date']; ?></td>
                                                <td id="print-btn" class="col-md-1"><?= $tabgon['modified_date']; ?></td>
                                                <td id="print-btn"  class="col-md-1.1">
                                                    <a href="edit-data.php?id=<?= $tabgon['id']; ?>" class="btn btn-success btn">Edit</a>
